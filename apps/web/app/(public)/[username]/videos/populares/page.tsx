...
export default function ChannelVideosPopularesPage() {
  ...
  useEffect(() => {
    api.get(`/api/users/${username}`).then(async (res) => {
      ...
      const r = await api.get(`/api/vods/?creatorId=${u.id}&sort=popular`)
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-semibold text-lg">Vídeos populares</h2>
      <VideoGrid vods={vods} emptyMessage="Sem vídeos populares" />
    </div>
  )
}
