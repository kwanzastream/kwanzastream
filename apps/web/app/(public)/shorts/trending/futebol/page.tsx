...
export default function ShortsTrendingFutebolPage() {
  const [shorts, setShorts] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    api.get("/api/shorts/trending?category=futebol").then(r => setShorts(r.data?.shorts || r.data || [])).catch(() => setShorts([])).finally(() => setLoading(false))
  }, [])
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><span className="text-2xl">⚽</span>Shorts — Futebol<Flame className="w-4 h-4 text-orange-500" /></h1>
          <p className="text-sm text-muted-foreground">Melhores momentos de futebol angolano.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          ...
          <p className="font-medium text-lg">Nenhum short de futebol disponível</p>
