...
const MOCK: LeaderboardEntry[] = Array.from({ length: 20 }, (_, i) => ({ rank: i + 1, username: `streamer-${i + 1}`, displayName: `Streamer ${i + 1}`, value: Math.round(500 - i * 20), valueLabel: "seguidores ganhos", change: Math.floor(Math.random() * 5) - 2 }))

export default function LeaderboardStreamersSeguidoresPage() {
  const [period, setPeriod] = useState("week")
  return (
    <div className="max-w-4xl mx-auto py-4 px-4 space-y-4">
      <h1 className="text-xl font-bold flex items-center gap-2"><Users className="w-5 h-5" />Top 100 — Seguidores Ganhos</h1>
      <LeaderboardTabs tabs={NAV} active="streamers" /><LeaderboardTabs tabs={METRICS} active="seguidores" /><LeaderboardPeriodSelector periods={PERIODS} active={period} onChange={setPeriod} />
